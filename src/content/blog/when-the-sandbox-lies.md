---
title: "The sandbox lied"
description: "An ERP production schema change broke our Entity Framework mapping."
pubDate: 2026-01-15
...
We did **not** try to redesign the whole thing while the system was down. Changing one column's mapping does not fix the larger fragility when *all* columns are treated this way.
...
But the part that hurt was ours: we chose a strictness level that could turn harmless drift into a full stop.
...
It is not about being adversarial with vendors. It is about knowing what you control and what you do not.
...
- **Detect schema drift explicitly**
  - Periodically check key tables and columns for drift across customer environments.
  - Raise a *warning* or a pager before the drift turns into a runtime failure.
  - Tradeoff: extra operational work and edge cases like permissions, latency, and tenant variance.

- **Allow reads, block writes when mismatch is detected**
  - If a mismatch is discovered, degrade to read-only behavior for the affected surface area.
  - Tradeoff: you avoid full outages, but you still lose some functionality.

- **Relax mapping constraints for "non-critical" fields**
...
- **Treat the sandbox as helpful, not authoritative**
  - Keep using it, but assume it can be stale.
  - Tradeoff: you will invest more in monitoring and defensive behavior.
